...
class TemplateCuisinistes(BaseTemplate):
    ...
    def generate(self, data: dict) -> Presentation:
        ...
        is_montigny = "montigny" in client.lower()
        # Montigny : pas de slide Google
        if is_montigny:
            has_google = False
        ...
        # Slide 2 — Récap
        is_cuisine_plus = "cuisine" in client.lower()
        if is_aviva:
            self._slide_recap_aviva(gen_curr, gen_prev, month_fr)
        else:
            # Cuisine Plus : itinéraires toujours, pas d'appels
            recap_iti = has_itineraires or is_cuisine_plus
            recap_appels = has_appels if not is_cuisine_plus else False
            self._slide_recap(gen_curr, gen_prev, c_curr, c_prev, month_fr,
                              recap_iti, recap_appels, has_leads)

        # Slide 2b — Évolution conversions
        evo_iti = has_itineraires or is_cuisine_plus
        evo_appels = has_appels if not is_cuisine_plus else False
        if has_history and (evo_iti or evo_appels):
            self._slide_evo_conversions(history, evo_iti, evo_appels)

        # Slide 3 — Google Ads
        if has_google:
            if is_cuisine_plus:
                self._slide_google_simple(g_curr, g_prev, month_fr)
            else:
            ...
            if has_history:
                ...
                elif "orgeval" in client.lower():
                    self._slide_evo_quad(history, [
                        ("Cout Google ADS", "Coût"),
                        ("Total Impressions", "Impressions"),
                        ("Clics Perf Max", "Clics PerfMax"),
                        ("Total CPC moyen", "CPC Moyen"),
                    ], "Google Ads")
                else:
                    self._slide_evo_quad(history, [
        ...
        # Slide 4 — Analytics pages (pas pour AvivA ni Cuisine Plus)
        if not is_aviva and not is_cuisine_plus:
            analytics_data = data.get("analytics", {})
            self._slide_analytics(gen_curr, gen_prev, month_fr, prev_month_fr, analytics_data)

        # Slide 5 — Meta Ads
        if has_meta:
            if is_aviva:
                self._slide_meta_no_ctr(m_curr, m_prev, month_fr)
            elif is_cuisine_plus:
        ...
        # Slide 7 — Synthèse
        if is_montigny:
            self._slide_synthese_montigny(gen_curr, gen_prev, month_fr)
        elif is_aviva:
            self._slide_synthese_aviva(gen_curr, gen_prev, g_curr, g_prev, m_curr, m_prev,
                                       ms_curr, ms_prev, month_fr, has_google, has_meta, has_microsoft)
        else:
            is_crozatier = "crozatier" in client.lower()
            self._slide_synthese(gen_curr, gen_prev, g_curr, g_prev, m_curr, m_prev,
                                 ...
                                 is_cuisine_plus=is_cuisine_plus, is_crozatier=is_crozatier)
    ...
    # ──────────────────────────────────────────
    # Slide 2 — Récap AvivA (Diffusion + Clics uniquement)
    # ──────────────────────────────────────────

    def _slide_recap_aviva(self, gen_curr, gen_prev, month_fr):
        slide = self.prs.slides.add_slide(self.blank_layout)
        ...
        self._hero_row(slide, row1, y=3.05, n_cols=2)
    ...
    def _slide_google_simple(self, g_curr, g_prev, month_fr):
        """Slide Google Ads simplifiée pour Cuisine Plus — Impressions + Clics totaux en pleine largeur."""
        slide = self.prs.slides.add_slide(self.blank_layout)
        self._modern_header(slide, "Détails Google Ads", month_fr, accent_color=PALETTE["green"])
    ...
    def _slide_analytics(self, gen_curr, gen_prev, month_fr, prev_month_fr, analytics_data=None):
        slide = self.prs.slides.add_slide(self.blank_layout)
        ...
        if ga_pages:
            ...
            page_rows = [(p, None, None) for p in ANALYTICS_PAGES]

        for label, val_curr, val_prev in page_rows:
            if val_curr is None:
    ...
    def _slide_meta_no_ctr(self, m_curr, m_prev, month_fr):
        """Slide Meta pour AvivA — sans CTR, layout 2x2."""
        slide = self.prs.slides.add_slide(self.blank_layout)
    ...
    def _slide_meta(self, m_curr, m_prev, month_fr):
        """Slide Meta 1/2 : Impressions, Clics, Coût — 3 cards centrées."""
        slide = self.prs.slides.add_slide(self.blank_layout)
        self._modern_header(slide, "Détails Meta Ads - Facebook et Instagram", month_fr,
                            accent_color=FUNCTIONAL_COLORS["meta_color"])
        ...
        row2 = [
            {"label": "CPC Moyen", "value": format_currency(cpc),
             "current": cpc, "previous": cpc_p, "tooltip": "CPC Meta", "accent": blue},
            {"label": "CTR Moyen", "value": format_percentage(ctr),
             "current": ctr, "previous": ctr_p, "tooltip": "CTR Meta", "accent": blue},
            {"label": "Leads Meta", "value": "—",
             "sub_label": "Nombre de leads générés via les publicités Meta", "accent": blue},
        ]
        self._hero_row(slide, row1, y=1.3, n_cols=3)
        self._hero_row(slide, row2, y=4.2, n_cols=3)

    def _slide_meta_2(self, m_curr, m_prev, month_fr):
        """Slide Meta 2/2 : CPC, CTR, Leads Meta — 3 cards centrées."""
        slide = self.prs.slides.add_slide(self.blank_layout)
        self._modern_header(slide, "Détails Meta Ads - Facebook et Instagram", month_fr,
                            accent_color=FUNCTIONAL_COLORS["meta_color"])
    ...
    def _slide_synthese_montigny(self, gen_curr, gen_prev, month_fr):
        """Synthèse Montigny — Achat Média Total + Diffusion totale uniquement."""
        slide = self.prs.slides.add_slide(self.blank_layout)
    ...
    def _slide_synthese_aviva(self, gen_curr, gen_prev, g_curr, g_prev, m_curr, m_prev,
                              ms_curr, ms_prev, month_fr, has_google, has_meta, has_microsoft):
        """Synthèse AvivA — Achat Total en haut, plateformes actives en bas."""
        slide = self.prs.slides.add_slide(self.blank_layout)
        self._modern_header(slide, "Synthèse", month_fr)
    ...
    def _slide_synthese(self, gen_curr, gen_prev, g_curr, g_prev, m_curr, m_prev,
                        ms_curr, ms_prev, c_curr, c_prev, month_fr,
                        has_google, has_meta, has_microsoft, has_leads,
                        is_cuisine_plus=False, is_crozatier=False):
        slide = self.prs.slides.add_slide(self.blank_layout)
        self._modern_header(slide, "Synthèse", month_fr)
        ...
        # Ligne 1 : Achat Total + Coût/formulaire (si données, sauf Cuisine Plus)
        row1 = [
            {"label": "Achat Média Total", "value": format_currency(cout_all),
             "current": cout_all, "previous": cout_all_p, "tooltip": "COUT ALL",
             "accent": PALETTE["gold"]},
        ]
        if not is_cuisine_plus:
            if is_crozatier:
                conv = self._safe_get(gen_curr, "Cout par conversion majeure")
                conv_p = self._safe_get(gen_prev, "Cout par conversion majeure")
                if conv:
                    row1.append({"label": "Coût / conversion majeure", "value": format_currency(conv),
                                 "current": conv, "previous": conv_p,
                                 "sub_label": "Coût total des campagnes divisé par le nombre de contacts et itinéraires",
                                 "accent": PALETTE["gold"]})
            else:
