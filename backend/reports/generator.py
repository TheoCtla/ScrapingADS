...
def generate_single_report(
    ...
    Args:
        client_name: Nom exact de l'onglet du Sheet (== nom client de l'allowlist).
        month: Mois cible en anglais (ex: 'February 2026'). Si None, M-1.
    """
    target_month = month or _compute_target_month()
    folder_name = _compute_drive_folder_name() if month is None else None
    ...
    logging.info(f"Génération rapport unique '{client_name}' pour '{target_month}' → dossier '{folder_name}'")
    ...
    try:
        ...
        if result["status"] == "success":
            logging.info(f"[OK] {client_name} → {result['filename']}")
        else:
            logging.info(f"[SKIP] {client_name} → {result.get('reason', '')}")
    except Exception as e:
        logging.error(f"[ERREUR] {client_name}: {e}", exc_info=True)
        result = {
            "client": client_name,
            "route": get_route_name(client_name),
            "status": "error",
            "error": str(e),
        }
